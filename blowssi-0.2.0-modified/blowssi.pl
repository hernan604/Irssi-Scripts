...
=head
### ATENTION: This is a modified blowssi.pl script. The modification is in method 'decrypt'. Its in the end. There was a signal_stop directive that made signals stop propagation, and my other scripts were not receiving events like 'message_public'. So i added a signal_continue directive. It seems to have worked but needs more testing.

and also i added the part below:
    elsif ($command =~ m/\/msg/i)
    {
      ( $channel, $message ) = $command_line =~ m#^/msg (\#[^ ]+) (.+)#ig; 
      print "ME: $channel | $message";
    }
which handles: 
    my $cmd    = "MSG ".$chan->{chan}." ".$text;
    my $server = Irssi::server_find_chatnet( $self->config->{ network } );
    $server->command( $cmd ) if defined $server;
=cut
...
sub encrypt
{
  ...
  { 
    ...
    elsif ($command =~ m/\/msg/i)
    {
      ...
      if ( $command_line =~ m#^/msg +?(-channel +)?(\#[^ ]+) (.+)#ig ) {
            ( $channel, $message ) = ( $2, $3 ); 
      }
    }
    else
...
# }
  ...
  return 0 if ! length $result;

 #use DDP; warn p @_;
  my @args_copy = @_;
  splice @args_copy, 2, 1, $result;
  my $type = shift @args_copy; #warn $type; # ie: message_public
 #warn p @args_copy;
  Irssi::signal_continue(@args_copy);
 #Irssi::signal_continue(@args_copy) if $type eq 'message_public';
  #Irssi::signal_stop();
  return 1;
}
...
Irssi::signal_add_first{
    ...
    "send text" => sub { encrypt 'send_text' => @_ },
    "send command" => sub {encrypt 'send_command' => @_},
  };
